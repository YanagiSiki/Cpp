...
void drawHead(snake_t snake[], short dir) {
    int x = snake[0].x;
    int y = snake[0].y;
    if (dir == UP)         { readimagefile(head_up, x, y, x+20, y+20);   }
    else if (dir == DOWN)  { readimagefile(head_down, x, y, x+20, y+20); }
    else if (dir == RIGHT) { readimagefile(head_right, x, y, x+20, y+20);}
    else if (dir == LEFT)  { readimagefile(head_left, x, y, x+20, y+20); }
}
...
int getDirect(snake_t snake[], short *dir, int *nx, int *ny) {
    ...
    if((*dir) == ESC) return -1;
    else if((*dir) == UP)   { (*ny)-=20;  }
    else if((*dir) == DOWN) { (*ny)+=20;  }
    else if((*dir) == LEFT) { (*nx)-=20;  }
    else if((*dir) == RIGHT){ (*nx)+=20;  }
    return 0;
}
...
void init(snake_t snake[], int *size, int nx, int ny) {
	snake[(*size)].x = nx; snake[(*size)].y=ny;    (*size)++;
	snake[(*size)].x = nx; snake[(*size)].y=ny+20; (*size)++;
}
void run() {
    food_t food;
    snake_t snake[20];
